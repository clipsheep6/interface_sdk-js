...
export default class UIServiceExtensionAbility extends ExtensionAbility {
  ...
  /**
   * Called back when an window stage will create.
   *
   * @param { window.WindowStageConfig } config - Indicates the window stage config.
   * @syscap SystemCapability.Ability.AbilityRuntime.Core
   * @systemapi
   * @stagemodelonly
   * @since 12
   */
  onWindowStageWillCreate(config: window.WindowStageConfig): void;

  /**
   * Called back when an window stage is created.
   *
   ...
   * @since 12
   */
  onWindowStageDidCreate(window: window.WindowStage): void
