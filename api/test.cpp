    ...
    const std::string &srcUri, const std::string &destUri, const NVal &listener, NVal copySignal)
{
    ...
    if (infos->copySignal) {
        ...
        if (taskSignalEntity != nullptr) {
            infos->taskSignal = taskSignalEntity->taskSignal;
        }
    }
    ...
    auto cbExec = [infos, callback]() -> NError {
        if (IsRemoteUri(infos->srcUri)) {
            auto ret = TransListener::CopyFileFromSoftBus(
                    infos->srcUri, infos->destUri, infos->sessionId, std::move(callback));
            if (infos->taskSignal != nullptr) {
                infos->taskSignal->MarkRemoteTask();
                infos->taskSignal->SetSessionIdOfRemoteTask(infos->sessionId);
            }
            return ret;
        }
