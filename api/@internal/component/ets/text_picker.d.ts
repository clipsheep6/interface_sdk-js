 ...
 * @since 12
 */
declare type onTextPickerChangeCallback = (value: string | string[], index: number | number[]) => void;
...
declare class TextPickerAttribute extends CommonMethod<TextPickerAttribute> {
   ...
   * Only valid when only text is displayed. When picture or picture plus text is displayed, the value is "".
   *
   * @param { Optional<onTextPickerChangeCallback> } handler - the callback of onChange.
   * @returns { TextPickerAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform
   * @atomicservice
   * @since 12
   */
  onChange(handler: Optional<onTextPickerChangeCallback>): TextPickerAttribute;
