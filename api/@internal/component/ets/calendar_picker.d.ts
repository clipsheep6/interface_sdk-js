 ...
 * @since 12
 */
declare type OnCalendarPickerChangeCallback = (value: Date) => void;
...
declare class CalendarPickerAttribute extends CommonMethod<CalendarPickerAttribute> {
  ...
  /**
   * Callback for selected date changed.
   * @param { Optional<OnCalendarPickerChangeCallback> } handler - callback for selected date changed.
   * @returns { CalendarPickerAttribute } the attribute of the CalendarPicker.
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform
   * @atomicservice
   * @since 12
   */
  onChange(handler: Optional<OnCalendarPickerChangeCallback>): CalendarPickerAttribute;
}
