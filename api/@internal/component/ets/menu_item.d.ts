 ...
 * 
 * @typedef { function } OnMenuItemChangeHandler
 ...
 * @since 12
 */
declare type OnMenuItemChangeHandler = (selected: boolean) => void;
...
declare class MenuItemAttribute extends CommonMethod<MenuItemAttribute> {
   ...
   * Triggers a callback when a menu item is selected or unchecked.
   *
   * @param { Optional<OnMenuItemChangeHandler> } handler
   * @returns { MenuItemAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform
   * @atomicservice
   * @since 12
   */
  onChange(handler: Optional<OnMenuItemChangeHandler>): MenuItemAttribute;
