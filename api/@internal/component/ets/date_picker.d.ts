 ...
 * @since 12
 */
declare type onDateChangeCallback =((value: Date) => void);
...
declare class DatePickerAttribute extends CommonMethod<DatePickerAttribute> {
   ...
   * This event is triggered when a DatePicker date or time is selected.
   *
   * @param { Optional<onDateChangeCallback> } handler
   * @returns { DatePickerAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform
   * @atomicservice
   * @since 12
   */
  onDateChange(handler: Optional<onDateChangeCallback>): DatePickerAttribute;
}
