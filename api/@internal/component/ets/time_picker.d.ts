 ...
 * @since 12
 */
declare type onTimePickerChangeCallback = (value: TimePickerResult) => void;
...
declare class TimePickerAttribute extends CommonMethod<TimePickerAttribute> {
  ...
  */
   ...
   * This event is triggered when a TimePicker time is selected.
   *
   * @param { Optional<onTimePickerChangeCallback> } handler
   * @returns { TimePickerAttribute }
   * @syscap SystemCapability.ArkUI.ArkUI.Full
   * @crossplatform
   * @atomicservice
   * @since 12
   */
  onChange(handler: Optional<onTimePickerChangeCallback>): TimePickerAttribute;
}
