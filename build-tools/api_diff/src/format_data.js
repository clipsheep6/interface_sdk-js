...
function mergeDataBetweenVersion(oldVersion, newVersion) {
...
// 合并前两个版本的数据，作为初始数据
function formatExcelData(orderVersionArr, url) {
  let initialData = [];
  let allMergeData = [];
  if (orderVersionArr.length === 1) {
    ...
    allMergeData = mergeAllData(initialData, orderVersionArr, url);
  }
  return allMergeData;
}
...
// 用初始数据跟其他版本的数据比较合并
function mergeAllData(initialData, orderVersionArr, url) {
  let number = 2;
...
function mergeDiffWithChangeLog(diffs, oldVersion, newVersion) {
  // TODO 与子系统changelog进行数据合并
  const afterMergeData = diffs.concat(mergeDataBetweenVersion(oldVersion, newVersion));
  return afterMergeData;
}
