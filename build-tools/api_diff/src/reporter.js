...
function reportDeletedApi(api) {
  return wrapApiDiffInfo(api, ApiStatusCode.DELETE, '', '');
}

function reportDeletedClass(api) {
...
function reportNewApi(api) {
  return wrapApiDiffInfo(api, ApiStatusCode.NEW_API, '', '', '');
}

function reportApiChanges(apiChanges) {
  return wrapApiDiffInfo(apiChanges.api, apiChanges.statusCode,
    `${apiChanges.oldMessage}`, `${apiChanges.newMessage}`, `${apiChanges.hint}`);
}

function wrapApiDiffInfo(api, statusCode, oldMessage, newMessage, hint) {
  const reporterData = new ReporterData(api);
...
class DiffReporter {
  ...
  addDeletedApi(api) {
    this.diffs.push(reportDeletedApi(api))
  }

  addDeletedPackage(packageName, dtsName) {
    this.diffs.push(reportDeletedPackage(packageName, dtsName));
  }

  addNewApi(api) {
    this.diffs.push(reportNewApi(api));
  }

  addChangedApi(apiChanges) {
    let reportItem;
    if (apiChanges.statusCode === ApiStatusCode.DELETE_CLASS) {
      reportItem = reportDeletedClass(apiChanges.api);
    } else {
      reportItem = reportApiChanges(apiChanges);
    }
    this.diffs.push(reportItem);
  }
