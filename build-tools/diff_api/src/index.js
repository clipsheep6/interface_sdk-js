...
const { JSONReporter, ExcelReporter, ChangelogReporter } = require('./api_writer');
...
exports.ApiDiffPlugin = {
  ...
  start: async function (argv) {
    ...
    const diffMap = await ApiDiffer.compareSdks(oldSdk, newSdk, oldVersion, newVersion);
    // const diffMap = await applyChangeLogs(diffInfoMap, oldVersion, newVersion)
  ...
  coverToArray(diffMap) {
    ...
    for (const data of values) {
      data.forEach(diffApi => {
        ...
        diffSet.add(JSON.stringify(...data));
      });
    }
