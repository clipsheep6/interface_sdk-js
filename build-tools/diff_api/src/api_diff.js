...
function getJSDocTagItem(api) {
  let curApi = api;
  while (curApi && (!curApi.jsdoc || curApi.jsdoc.length === 0)) {
    curApi = curApi.parent;
  }
  return curApi;
}
 ...
 * @param {*} ext 
 * @param {*} sameApiNameNumber 与旧版本API名字相同的新版本API个数
 */
function getFunctionDiff(oldApis, newClassMap, ext, sameApiNameNumber) {
  if (sameApiNameNumber === 1) {
    newClassMap.forEach((apiDigestInfo, apiSignautre) => {
      ext.diffReporter.addChangedApi({
        api: oldApis[0],
        statusCode: StatusCode.FUNCTION_CHANGES,
        oldMessage: oldApis[0].getRawText(),
        newMessage: apiDigestInfo[0].getRawText(),
        hint: '',
        oldNode: oldApis[0].node,
        newNode: apiDigestInfo[0].node,
        syscap: getSycap(apiDigestInfo[0])
      })
      ext.diffReporter.addDiffInfo({
        api: oldApis[0],
        statusCode: StatusCode.FUNCTION_CHANGES,
        oldMessage: oldApis[0].getRawText(),
        newMessage: apiDigestInfo[0].getRawText(),
    ...
    if (newApiTypeMap.get(oldApiType) !== undefined && newApiTypeMap.get(oldApiType).size > 1) {
      ext.diffReporter.addDeletedApi(oldApis[0], getSycap(oldApis[0]));
      newApiTypeMap.get(oldApiType).forEach(apiSignautre => {
        if (!newClassMap.get(apiSignautre)) {
          ext.diffReporter.addNewApi(newClassMap.get(apiSignautre)[0], getSycap(newClassMap.get(apiSignautre)[0]));
          newClassMap.delete(apiSignautre);
        }
      })
      ...
      ext.diffReporter.addChangedApi({
        ...
        statusCode: StatusCode.FUNCTION_CHANGES,
        oldMessage: oldApis[0].getRawText(),
        newMessage: newClassMap.get(...newApiTypeMap.get(oldApiType))[0].getRawText(),
        hint: '',
        oldNode: oldApis[0].node,
        newNode: newClassMap.get(...newApiTypeMap.get(oldApiType))[0].node,
        syscap: getSycap(newClassMap.get(...newApiTypeMap.get(oldApiType))[0])
      })
      ext.diffReporter.addDiffInfo({
        api: oldApis[0],
        statusCode: StatusCode.FUNCTION_CHANGES,
...
function getEveryNewApiType(newClassMap, newApiTypeMap) {
  let callbackApiSignautreSet = new Set();
  let promiseApiSignautreSet = new Set();
  let otherTypeApiSignautreSet = new Set();
 ...
 * @returns 
 */
async function compareSdks(oldDir, newDir, oldVersion, newVersion) {
