    ...
    if (ts.isFunctionDeclaration(node) || ts.isMethodDeclaration(node) || ts.isMethodSignature(node)) {
      ...
      if ((node.parameters[0].type) && (node.parameters[0].type.kind === ts.SyntaxKind.UnionType)) {
        ...
        for (let i = 0; i < types.length; i++) {
          const eventValue: ts.TypeNode = types[i];
          if (eventValue.kind === ts.SyntaxKind.LiteralType && (eventValue as ts.LiteralTypeNode).literal.kind === ts.SyntaxKind.StringLiteral) {
            const str = (eventValue as ts.LiteralTypeNode).literal.getText();
            // 这里获取的str包含了单引号或双引号，因此需要进行截取
            eventValues.push(str.slice(1, str.length - 1));
          } else {
            return [];
          }
        }
  ...
  splitEventSubscriptionFunction(node: ts.Node) {
    ...
    if (ts.isFunctionDeclaration(node)) {
      ...
      eventValues.forEach((eventValue) => {
        ...
        const newFunctionNode: ts.FunctionDeclaration = ts.factory.createFunctionDeclaration(node.modifiers, node.asteriskToken, node.name, node.typeParameters,
        ...
        CommentHelper.setComment(newFunctionNode, comments);
