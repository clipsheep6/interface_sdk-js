...
# 子系统对应关系
system_dict = {'js-apis-DataUriUtils.md': '元能力', 'js-apis-featureAbility.md': '元能力',
...
# 不进行自动化的文档
no_test = ['js-apis-data-resultset.md', 'js-apis-application-MissionSnapshot.md',
...
def special(code, md_path, special_input, special_output):
    if 'js-apis-distributedMissionManager.md' in md_path and 'remoteDeviceId' in code:
    ...
    elif 'js-apis-ability-context.md' in md_path or 'js-apis-application-context.md' in md_path \
            or 'js-apis-service-extension-context.md' in md_path:
        if 'context' in code:
            code = code.replace('this.context', 'globalThis.context')

    elif 'js-apis-camera.md' in md_path:
        if 'let profile =' not in code:
            code = code.replace('profile',
                                '{format: camera.CameraFormat.CAMERA_FORMAT_RGBA_8888, size:{height: 100,width:100}}')\
                .replace(
                'surfaceId', '"surfaceId"')
        else:
        ...
        if special_output:
            ...
            for i in range(len(import_code)):
                index = code.find('\n')
                code = code[index + 1:]
            specail_code1 = 'cameraManager.getSupportedCameras().then((cameras) => {\n    let cameraDevice = cameras[' \
                            '0];\n    cameraManager.getSupportedOutputCapability(cameraDevice).then((' \
...
def data_rdb(code, md_path):
    if 'js-apis-data-rdb.md' in md_path:
        import_code = "import dataShare from '@ohos.data.dataShare'\n"
        if "import dataShare from '@ohos.data.dataShare'" in code:
            code = code.replace(import_code, '')
            code = import_code + code
    return code


def instructions_for_use(code, instructions_for_use_code):
    import_code = re.findall('import.+from.+\n', code)
    print('导入模块', import_code)
    for i in range(len(import_code)):
        index = code.find('\n')
        code = code[index + 1:]
...
def repeat_definition(code, md_path, class_name):
    if 'js-apis-distributed-data.md' in md_path:
        md_list = code.split('\n')
        del md_list[-1]
        let_name = 'let' + ' ' + class_name
        var_name = 'var' + ' ' + class_name
        const_name = 'const' + ' ' + class_name
        del_code = ''
        for i in range(5):
            if let_name in md_list[i] or var_name in md_list[i] or const_name in md_list[i]:
                del_code = md_list[i]
                break
