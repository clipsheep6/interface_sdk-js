...
class Project {
  ...
  getAppSdkPath() {
    ...
    if (!fs.existsSync(localPropertiesPath)) {
      Logger.error(this.logTag, `unable to get the sdk path of the project, specify it using the --sdk command option`);
      return this.sdkPath;
    }
  ...
  getModuleSource(modulePath, isIncludeTest) {
    const sourceSets = ['src/main/ets', 'src/main/js'];
    if (isIncludeTest) {
      sourceSets.push(...['src/ohosTest/ets', 'src/ohosTest/js']);
    }
    const sources = [];
    sourceSets.forEach((sourcePath) => {
      ...
      if (fs.existsSync(srcPath)) {
        ...
        FileSystem.listFiles(srcPath, (filePath) => {
          ...
          return fileName.endsWith('.ts') || fileName.endsWith('.ets') || fileName.endsWith('.js');
        }, sources);
      }
    });
    if (sources.length === 0) {
...
class Sdk {
  ...
  getPath() {
    ...
    const sdkDir = this.project.getAppSdkPath();
    if (sdkVersion && sdkDir) {
      this.sdkPath = path.resolve(sdkDir, `${sdkVersion}`, 'ets');
    }
    return this.sdkPath;
  }
...
class FileSystem {
  ...
  static listAllAppDirs(parentDir) {
    ...
    this.listDirectory(parentDir, dest, (filePath) => {
      ...
      return fs.existsSync(buildProfilePath);
    }, (filePath) => {
      return filePath;
    });
    return dest;
