...
class SystemApiRecognizer {
  ...
  /**
   * 遍历所有SourceFile集合，搜集指定信息
   * 
   * @param {ts.SourceFile} sourceFile 
   */
  visitSourceFile(sourceFile) {
    if (path.basename(sourceFile.fileName) === 'common.d.ts') {
      this.setCommonSourceFile(sourceFile);
    }
    const isCompoentSourceFile =
      isInDirectory(path.resolve(this.systemRoot, 'component'), sourceFile.fileName);
    if (isCompoentSourceFile) {
      this.addComponentSourceFile(sourceFile);
    }
  }

  addComponentSourceFile(sourceFile) {
    if (!this.componentSourceFiles) {
      this.componentSourceFiles = [];
    }
    this.componentSourceFiles.push(sourceFile);
  }
  ...
  setCommonSourceFile(sourceFile) {
    this.commonSourceFile = sourceFile;
  }

  getCommonMethodTable() {
    if (!this.commonMethods) {
      this.commonMethods = this.commonSourceFile.locals.get('CommonMethod');
    }
  ...
  isBuildMethodInStruct(node) {
    return this.arkUIRender.has(node.name.getText()) && ts.isStructDeclaration(node.parent);
  }
  ...
  recognizeHeritageClauses(node, fileName) {
    ...
    let heritagePropertyMap = this.collectAllHeritageMethods(node);
    if (heritagePropertyMap.size === 0) {
      const extendClassName = nodeSymbol.valueDeclaration.heritageClauses[0].types[0].expression.escapedText;
      heritagePropertyMap = this.getExtendClassPropertyMap(node, extendClassName);
    }
    if (!heritagePropertyMap) {
      return;
    }
  ...
  getExtendClassPropertyMap(node, extendClassName) {
    let extendClassPropertyMap = new Map();
    node.parent.statements.forEach(statement => {
      if (ts.isClassDeclaration(statement) && statement.name.escapedText === extendClassName) {
        extendClassPropertyMap = this.collectAllHeritageMethods(statement);
        const nodeType = this.typeChecker.getTypeAtLocation(statement);
        const nodeSymbol = nodeType.getSymbol();
        const heritageClauses = nodeSymbol.valueDeclaration.heritageClauses;
        while (heritageClauses !== undefined && extendClassPropertyMap && extendClassPropertyMap.size === 0) {
          const extendClassName = heritageClauses[0].types[0].expression.escapedText;
          extendClassPropertyMap = this.getExtendClassPropertyMap(statement, extendClassName);
        }
      }
    })
    return extendClassPropertyMap;
  }
  ...
  collectAllHeritageMethods(node) {
    ...
    node.heritageClauses.forEach((heritage) => {
      heritage.types.forEach((child) => {
        ...
        if (!this.isSdkApi(valueDeclaration.getSourceFile().fileName)) {
          return;
        }
  ...
  getSdkApiFromValueDeclaration(valudeDec) {
    ...
    apiInfo.setApiSourceFile(sourceFile);
...
class ApiDeclarationInformation {
  ...
  setApiSourceFile(sourceFile) {
    this.apiSourceFile = sourceFile;
  }
