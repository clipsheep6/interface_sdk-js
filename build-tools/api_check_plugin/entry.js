...
function checkEntry(url) {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // result += `url:::${url} \n`;
      // result += '============================\n';
      // result += `原文:::${JSON.stringify(res)} \n`;
      // result += '============================\n';
      // result += `评论:::${resBody}\n`;
      // result += '============================\n';
      // result += `用户配置：：-------------\n`;
      // administrators.forEach((val)=>{
      //   result += `${val}\n`;
      // })
      // const keyStr = 'pass api check';
      let comments = JSON.parse(`{"resultBody": ${resBody}}`);
      // result += '============================\n';
      // result += `用户：：${JSON.stringify(comments.resultBody)}-------------\n`;
      let resultBody = comments.resultBody
      // result += `类型：：：${resultBody instanceof Array}`
      for (let i = 0; i < comments.resultBody.length; i++) {
        const comment = comments.resultBody[i];
        // result += `第${i}次：：============================\n`;
        // result += `flag：：${administrators.has(String(comment['user']['id'])) && /^approve api check$/.test(comment.body)},userid:${comment['user']['id']},comment.body:${comment.body}============================\n`;
        // result += `用户ID::${JSON.stringify(comment['user'])}`;
    ...
    // result += `isOpenEscapeWay:${isOpenEscapeWay}`
