...
function checkEntry(prId) {
  ...
  try {
    ...
    const { scanEntry, reqGitApi, getMdFiles } = require(path.resolve(__dirname, './src/api_check_plugin'));
    ...
    buffer = execSync(`cd interface/sdk-js/build-tools/dts_parser/package && node ./JS_API_CHECK.js -N checkOnline --path ${filePath} --checker all  --output ${resultPath} --excel false`, {
      timeout: 120000,
    });
    newToolResult.push(`resultPath:${resultPath}`);
    newToolResult.push(`resultJsonPath:${path.resolve(__dirname, resultPath)}`);
    if (fs.existsSync(path.resolve(__dirname, resultPath))) {
      ...
      newToolResultArr.forEach(newToolResultInfo => {
        ...
        newToolResult.push('filePath==',filePath)
