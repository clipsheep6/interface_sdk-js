...
const { getApiInfo, ErrorType, ErrorLevel, LogType, eventSubscriptionAPICheckSince, requireTypescriptModule } = require('./utils');
...
function checkOffFunctions(nodes, sourcefile, fileName) {
  let hasOffWithoutLastParameter = false;
  let hasOffLastParameterNotOptional = false;
  for (let node of nodes) {
    if (node.parameters.length > 1) {
      ...
      if (lastParameter.type.kind !== ts.SyntaxKind.JSDocNullableType) {
        hasOffLastParameterNotOptional = true
      }
    } else {
      hasOffWithoutLastParameter = true;
    }
  }
  if (hasOffLastParameterNotOptional && !hasOffWithoutLastParameter) {
    ...
    addAPICheckErrorLogs(nodes[0], sourcefile, fileName, ErrorType.PARAMETER_ERRORS, checkErrorResult, LogType.LOG_API, 
      ErrorLevel.MIDDLE);
  } else if (hasOffLastParameterNotOptional && hasOffWithoutLastParameter) {
    // 这种特殊情况不算错误，但会输出提示
