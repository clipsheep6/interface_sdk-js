...
function checkApiNaming(node, sourcefile, fileName) {
  ...
  for (const [key, value] of lowercaseNamingMap) {
    ...
    const lowercaseIgnoreWordArr = value.ignore.map((word) =>
      word.toLowerCase()
    );
    if (
      ...
      const internalWord = node
        .getText()
        .substr(prohibitedWordIndex, key.length);
      const errorInfo = `Prohibited word in [${node.getText()}]:{${internalWord}}.The word allowed is [${
        value.suggestion
      }]`;
  ...
  for (const [key, value] of lowercaseNamingScenarioMap) {
    ...
    if (
      ...
      const internalWord = node
        .getText()
        .substr(prohibitedWordIndex, key.length);
      const errorInfo = `Prohibited word in [${node.getText()}]:{${internalWord}} in the [${path.basename(
        fileName
      )}] file`;
      addAPICheckErrorLogs(
        node,
