...
function getParentApiInfo(node, noteStr, note) {
  ...
  apiInfo.formInfo = matchLabelInfo(node, noteStr, note).formInfo;
  return apiInfo;
}
...
async function getExcelBuffer(api) {
  ...
  for (let i = 1; i <= api.length; i++) {
    ...
    apiData.version, apiData.isSystemApi, apiData.formInfo]
  }
