  ...
  return (context) => {
    return (node) => {
      ...
      statements.forEach(stat => {
        ...
        if (ts.isExportAssignment(stat)) {

        } else if (ts.isImportDeclaration(stat)) {

        } else if (ts.isInterfaceDeclaration(stat)) {
          collectInterfaceDeclaration(stat, packageName, api, exportApi, returnDeclarationArr, hash, apiInfo, currentTypeList, dtsPath);

        } else if (ts.isModuleDeclaration(stat)) {
          collectModuleDeclaration(stat, packageName, api, exportApi, returnDeclarationArr, hash, apiInfo, currentTypeList, dtsPath);

        } else if (ts.isClassDeclaration(stat)) {
          collectClassDeclaration(stat, packageName, api, exportApi, returnDeclarationArr, hash, apiInfo, currentTypeList, dtsPath);

        } else if (ts.isEnumDeclaration(stat)) {
          collectEnumDeclaration(stat, packageName, api, exportApi, returnDeclarationArr, hash, apiInfo, currentTypeList, dtsPath);
          ...
          if (ts.isMethodDeclaration(stat) || ts.isMethodSignature(stat) || ts.isFunctionDeclaration(stat)) {
            var methodName = stat.name.getText()
            let className = '';
...
function getApiInfo(node, apiInfos) {
  ...
  if (notesStr !== "" && noteArr.length <= 2) {
    ...
    const note = 'two';
    // const oldNoteStr = noteArr[1];
    const newNoteStr = noteArr[2];
    // const oldApiInfo = getParentApiInfo(node, oldNoteStr, note);
    return getParentApiInfo(node, newNoteStr, note);
    // let info = {
    //   isSystemApi: { old: oldApiInfo.isSystemApi, new: newApiInfo.isSystemApi },
    //   version: { old: oldApiInfo.version, new: newApiInfo.version },
    //   deprecated: { old: oldApiInfo.deprecated, new: newApiInfo.deprecated },
    //   model: { old: oldApiInfo.model, new: newApiInfo.model },
    //   sysCap: { old: oldApiInfo.sysCap, new: newApiInfo.sysCap },
    //   permission: { old: oldApiInfo.permission, new: newApiInfo.permission },
    //   useinsteadInfo: { old: oldApiInfo.useinsteadInfo, new: newApiInfo.useinsteadInfo },
    //   errorCode: { old: oldApiInfo.errorCode, new: newApiInfo.errorCode },
    //   typeInfo: { old: oldApiInfo.typeInfo, new: newApiInfo.typeInfo },
    //   note: 'two'
    // }
    return info;
  } else if (notesStr === "") {
...
function matchLabelInfo(node, noteParagraph, note) {
  ...
  if (/\@[D|d][E|e][P|p][R|r][E|e][C|c][A|a][T|t][E|e][D|d].*[S|s][I|i][N|n][C|c][E|e]\s*(\d+)/g.test(notesStr)) {
    notesStr.replace(/\@[D|d][E|e][P|p][R|r][E|e][C|c][A|a][T|t][E|e][D|d].*[S|s][I|i][N|n][C|c][E|e]\s*(\d+)/g,
      deprecatedInfo => {
        noteInfo.deprecated = deprecatedInfo.replace(
          /\@[D|d][E|e][P|p][R|r][E|e][C|c][A|a][T|t][E|e][D|d].*[S|s][I|i][N|n][C|c][E|e]\s*/g, '').trim();
      })
  } else {
    noteInfo.deprecated = 'N/A';
  }
  if (/\@[D|d][E|e][P|p][R|r][E|e][C|c][A|a][T|t][E|e][D|d]\s*(\d+)/g.test(notesStr)) {
    notesStr.replace(/\@[D|d][E|e][P|p][R|r][E|e][C|c][A|a][T|t][E|e][D|d]\s*(\d+)/g,
      deprecatedInfo => {

        noteInfo.deprecated = deprecatedInfo.replace(
          /\@[D|d][E|e][P|p][R|r][E|e][C|c][A|a][T|t][E|e][D|d]\s*/g, '').trim();
      })
  }
  ...
  if (/\@[P|p][E|e][R|r][M|m][I|i][S|s][S|s][I|i][O|o][N|n]\s*((\w|\.|\/|\{|\@|\}|\s)+)/g.test(notesStr)) {
    notesStr.replace(/\@[P|p][E|e][R|r][M|m][I|i][S|s][S|s][I|i][O|o][N|n]\s*((\w|\.|\/|\{|\@|\}|\s)+)/g,
      permissionInfo => {
        noteInfo.permission = permissionInfo.replace(/\@[P|p][E|e][R|r][M|m][I|i][S|s][S|s][I|i][O|o][N|n]/g, '').trim();
      })
  } else {
    noteInfo.permission = 'N/A';
  }
  if (/\@throws { BusinessError } \d{2,18}/g.test(notesStr)) {
    ...
    notesStr.replace(/\@throws { BusinessError } \d{2,18}/g, (code) => {
      ...
      noteInfo.errorCode = errorCode;
    })
  } else {
    noteInfo.errorCode = 'N/A';
  }
...
function getParentApiInfo(node, noteStr, note) {
  let apiInfo = {};
  // if (!apiInfo.version) {
  //   if (matchLabelInfo(node, noteStr, note).version) {
  //     apiInfo.version = matchLabelInfo(node, noteStr, note).version;
  //   } else if (node.parent) {
  //     apiInfo.version = getParentApiInfo(node.parent, noteStr,apiInfo).version;
  //   } else {
  //     apiInfo.version = 'N/A';
  //   }
  // }
  ...
  // if (!apiInfo.model) {
  //   if (matchLabelInfo(node, noteStr, note).model) {
  //     apiInfo.model = matchLabelInfo(node, noteStr, note).model;
  //   } else if (node.parent) {
  //     apiInfo.model = getParentApiInfo(node.parent,noteStr, apiInfo).model;
  //   }else{
  //     apiInfo.model = 'N/A';
  //   }
  // }
  ...
  //   if (matchLabelInfo(node, noteStr, note).isSystemApi) {
  //     apiInfo.isSystemApi = matchLabelInfo(node, noteStr, note).isSystemApi;
  //   } else if (node.parent) {
  //     apiInfo.isSystemApi = getParentApiInfo(node.parent, noteStr,apiInfo).isSystemApi;
  //   }
  // }
  if (!apiInfo.sysCap) {
  ...
  // if (!apiInfo.permission) {
  //   if (matchLabelInfo(node, noteStr, note).permission) {
  //     apiInfo.permission = matchLabelInfo(node, noteStr, note).permission;
  //   } else if (node.parent) {
  //     apiInfo.permission = getParentApiInfo(node.parent, noteStr,apiInfo).permission;
  //   } else {
  //     apiInfo.permission = 'N/A';
  //   }
  // }
